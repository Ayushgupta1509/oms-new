...
import { useLoginwithphonenumberMutation } from '../../services/userAuthApi';
const Loginwithphonenumber = () => {
  const [server_error, setServerError] = useState({})
  const navigate = useNavigate();
  const [loginwithphonenumber, { isLoading }] = useLoginwithphonenumberMutation()
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await loginwithphonenumber(actualData)
    if (res.error) {
  ...
  <Grid container justifyContent='center' >
    ...
    <Box style={{width:"100%"}}  component='form' noValidate sx={{ mt: 1 }} id='login-form' onSubmit={handleSubmit}>
